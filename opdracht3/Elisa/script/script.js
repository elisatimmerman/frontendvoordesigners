...
function showData(jsonObj) {
    ...
    h2.textContent = jsonObj['title'];
    header.appendChild(h2);
  
    var simple_plot = document.createElement('p');
    simple_plot.textContent = 'Hometown: ' + jsonObj['simple_plot'];
    header.appendChild(simple_plot);
  }
  ...
  function films(jsonObj) {
    ...
    for (var n = 0; n < genres.length; n++) {
      ...
      filmBekijken.appendChild(filmplot);
      filmBekijken.appendChild(filmcover);
...
//         html += '<img id=' + json[i].id + ' title=' + json[i].title + ' simple_plot=' + json[i].simple_plot + '>';
//     }
